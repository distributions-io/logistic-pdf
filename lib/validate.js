...
* @param {Number} [options.s=1] - scale parameter
...
function validate( opts, options ) {
	...
	if ( options.hasOwnProperty( 's' ) ) {
		opts.s = options.s;
		if ( !isPositive( opts.s ) ) {
			return new TypeError( 'pdf()::invalid option. `s` parameter must be a positive number. Option: `' + opts.s + '`.' );
		}
	}
