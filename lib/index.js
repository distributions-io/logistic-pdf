...
* @param {Number} [opts.s=1] - scale parameter
...
function pdf( x, options ) {
	...
	opts.s = typeof opts.s !== 'undefined' ? opts.s : 1;

	if ( isNumber( x ) ) {
		return pdf1( x, opts.mu, opts.s );
	}
	if ( isMatrixLike( x ) ) {
		if ( opts.copy !== false ) {
		...
		return pdf5( out, x, opts.mu, opts.s );
	}
	if ( isTypedArrayLike( x ) ) {
		...
		return pdf6( out, x, opts.mu, opts.s );
	}
	if ( isArrayLike( x ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return pdf4( x, opts.mu, opts.s, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return pdf3( out, x, opts.mu, opts.s, opts.accessor );
		}
		return pdf2( out, x, opts.mu, opts.s );
	}
	return NaN;
} // end FUNCTION pdf()
